...
function analyzeDirectory(targetPath) {
    ...
    return { detectedLangs, detectedFrameworks };
}
...
async function main() {
    rl.question('\n📂 Enter the path of the project to analyze (Press Enter to skip): ', (answer) => {
        const targetPath = answer.trim();

        if (!targetPath) {
            console.log("⚠️ Skipping analysis. Using generic settings.");
            ...
            console.log(`✅ Default context written to ${OUTPUT_FILE}`);
            rl.close();
            return;
        }
